...
const TOPICS = [
  ...
  {
    id: "modals", group: "struktury", title: "Modal Verbs", color: "#F59E0B", icon: "â",
    intro: "Po modal verb zawsze goĹe infinitive â bez 'to', bez -s, bez -ed.",
    data: [
      { group: "MOĹťLIWOĹÄ / ZDOLNOĹÄ", items: [
      ...
      { group: "ZAKAZ / BRAK KONIECZNOĹCI", items: [
  ...
  },
  {
    ...
    data: [
      ...
      { type: "3", name: "Third Conditional", when: "Nierealna sytuacja w PRZESZĹOĹCI â za pĂłĹşno", formula: "If + Past Perfect â would have + pp", ex: "If I had studied, I would have passed.", pl: "Gdybym siÄ uczyĹ, zdaĹbym.", tip: "Ĺťal za przeszĹoĹciÄ. Nie da siÄ zmieniÄ." },
    ]
  },
  {
  ...
  {
    id: "relative", group: "struktury", title: "Relative Clauses", color: "#E879F9", icon: "â",
    ...
    data: {
      ...
      tip: "MoĹźesz opuĹciÄ who/which/that gdy jest OBIEKTEM:\n'The book (that) I read' â â ale 'The man who called me' â nie moĹźna opuĹciÄ (who = podmiot)"
    }
  },
  ...
  {
    id: "linking", group: "zdania", title: "Linking Words", color: "#34D399", icon: "âś",
  ...
  {
    id: "wish", group: "zdania", title: "Wish / If only", color: "#E879F9", icon: "âŚ",
    intro: "WyraĹźanie Ĺźalu i niespeĹnionych ĹźyczeĹ. Wish dziaĹa jak conditionals â czas cofa siÄ wstecz.",
    data: [
      { type: "Wish + Past Simple", when: "Ĺťyczenie o TERAĹšNIEJSZOĹCI (nierealne)", ex: "I wish I knew the answer.", pl: "Szkoda, Ĺźe nie znam odpowiedzi.", tip: "NIE 'I wish I know' â zawsze Past Simple dla teraĹşniejszoĹci" },
      { type: "Wish + Past Perfect", when: "Ĺťal za PRZESZĹOĹCIÄ (za pĂłĹşno)", ex: "I wish I had studied harder.", pl: "Szkoda, Ĺźe nie uczyĹem siÄ bardziej.", tip: "Nie moĹźna zmieniÄ â ĹźaĹujesz przeszĹoĹci" },
  ...
  },
  {
    ...
    data: [
      ...
      { rule: "â (brak)", color: "#A78BFA", when: "Rzeczowniki niepoliczalne i liczba mnoga w sensie ogĂłlnym.", items: [
        ...
        <div key={group.group} style={{ marginBottom: 20 }}>
          ...
          {group.items.map((t, i) => (
            <Card key={i} id={`t-${group.group}-${i}`} open={open} setOpen={setOpen} color={group.color}
              ...
              <div style={s.exBox}><div style={{ color: "#7ec8a0", marginBottom: 4 }}>â {t.ex}</div><div style={{ color: "#e87070" }}>â {t.neg}</div></div>
              {t.tip && <div style={s.note(group.color)}>đĄ {t.tip}</div>}
            </Card>
          ))}
        </div>
      ))}
        ...
        <div key={gi} style={{ marginBottom: 18 }}>
          ...
          {group.items.map((item, ii) => (
            <Card key={ii} id={`m-${gi}-${ii}`} open={open} setOpen={setOpen} color={topic.color}
              ...
              <div style={{ ...s.mono("#7ec8a0"), marginBottom: item.tip ? 8 : 0 }}>â {item.ex}</div>
              {item.tip && <div style={s.note(topic.color)}>đĄ {item.tip}</div>}
            </Card>
          ))}
        </div>
      ))}
...
function ReportedTopic({ topic, open, setOpen }) {
  ...
  return (
    <div>
      ...
      <div style={s.note(topic.color)}>đĄ {c.tip}</div>
    </div>
  );
}
    ...
    <div>
      ...
      {c.pronouns.map((p, i) => (
        <Card key={i} id={`rl-${i}`} open={open} setOpen={setOpen} color={topic.color}
          ...
          <div style={{ ...s.mono("#7ec8a0"), marginBottom: p.tip ? 8 : 0 }}>â {p.ex}</div>
          {p.tip && <div style={s.note(topic.color)}>đĄ {p.tip}</div>}
        </Card>
      ))}
      ...
      {c.types.map((t, i) => (
        <Card key={`rt-${i}`} id={`rt-${i}`} open={open} setOpen={setOpen} color={topic.color}
          ...
          <div style={s.note(topic.color)}>đĄ {t.tip}</div>
        </Card>
      ))}
      <div style={s.note(topic.color)}>đĄ {c.tip}</div>
    </div>
        ...
        <Card key={bi} id={`a-${bi}`} open={open} setOpen={setOpen} color={block.color}
          ...
          {block.items.map((item, ii) => (
            <div key={ii} style={{ display: "flex", gap: 8, marginBottom: 7 }}>
              <span style={{ color: "#7ec8a0", fontFamily: "monospace" }}>â</span>
              <div><span style={s.mono("#ccc")}>{item.ex}</span><span style={{ fontSize: 11, color: "#555", marginLeft: 8 }}>â {item.n}</span></div>
            </div>
          ))}
          {block.tip && <div style={s.note(block.color)}>đĄ {block.tip}</div>}
        </Card>
      ))}
    </div>
  );
        ...
        <div key={gi} style={{ marginBottom: 18 }}>
          ...
          {group.items.map((item, ii) => (
            <Card key={ii} id={`q-${gi}-${ii}`} open={open} setOpen={setOpen} color={group.color}
              header={<div style={{ display: "flex", alignItems: "center", gap: 10 }}><span style={{ ...s.mono(group.color, 13), fontWeight: "bold" }}>{item.word}</span><span style={{ fontSize: 11, color: "#555" }}>â {item.use}</span></div>}>
              <div style={s.mono("#7ec8a0")}>{item.ex}</div>
              {item.tip && <div style={s.note(group.color)}>đĄ {item.tip}</div>}
            </Card>
          ))}
        </div>
      ))}
    ...
    <div>
      ...
      {topic.data.map((item, i) => (
        <Card key={i} id={`u-${i}`} open={open} setOpen={setOpen} color={topic.color}
          ...
          <div style={s.note(topic.color)}>đĄ {item.tip}</div>
        </Card>
      ))}
    </div>
  );
